...
// console.log(person.name); // 'SungAh'
...
// parson이라는 객체에서 name과 age라는 key값을 분해한 값을 let으로 선언한 변수로 만든다
let {name,age} = person
console.log(name,age)
console.log(name)
...
// rest대신에 다를 이름을 사용해도 상관없음
// 변수 앞에 '...' 변수가 가장 마지막에 위치 필요
// 구조 분해 할당
let [a, b, ...rest] = arr;
console.log(rest); // 출력값: [3, 4, 5]